#output::::
#counter : 5.0
#  0.157878 seconds (197.16 k allocations: 7.533 MiB)
...
function mort(proj, policy, mortality, prev_model)
    try
        return mortality[trunc(Int, policy[2] + trunc(Int, proj/12)), 2]
    catch
        return prev_model[4]
    end
end
...
function lapserate(proj, policy, lapse, prev_model)
    try
        return lapse[trunc(Int, trunc(Int, policy[5] + proj - 1)/12) + 1, 2]/12
    catch
        try
            return prev_model[5]
        catch
...
function main()
    filepath = "openASSA"
    filename = "Simple Life Model.xlsm"
